...
export default function StudentDashboard() {
  ...
  useEffect(() => {
    studentAPI.getAttendance(fromDate, toDate).then(r => setAttendance(r.data)).catch(() => {})
    studentAPI.getFees().then(r => setFees(r.data)).catch(() => {})
  }, [])
      ...
      <div className="card mb-5">
        ...
        <div className="flex items-center gap-4">
          <div className="flex-1 bg-slate-100 rounded-full h-3 overflow-hidden">
            <div
              className={`h-full rounded-full transition-all duration-700 ${percentage >= 75 ? 'bg-emerald-500' : 'bg-rose-500'}`}
              style={{ width: `${percentage}%` }}
            />
          </div>
          <span className={`font-display text-lg font-bold ${percentage >= 75 ? 'text-emerald-600' : 'text-rose-600'}`}>
            {percentage}%
          </span>
        </div>
        {percentage < 75 && (
